...
def recalculaDist(dist, part1, part2, partitions):
    i = 0
    while i < part2:
        if dist[part1][i] < dist[part2][i]:
            dist[part2][i] = dist[part1][i]
        i+=1
    i+=1
    while i < part1:
        if dist[part1][i] < dist[i][part2]:
            dist[i][part2] = dist[part1][i]
        i+=1
    i+=1
    while i <= len(partitions):
        if dist[i][part1] < dist[i][part2]:
            dist[i][part2] = dist[i][part1]
        i+=1
    for j in range(len(partitions), part1, -1):
        dist[j].pop(part1)
    dist.pop(part1)
    return dist
