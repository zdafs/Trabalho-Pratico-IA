...
def associa_objeto(coordinate, partitions, add):
    ...
    coordinate.partition = min_index + add
    partitions[min_index].nk += 1

def recalcula_centroides(coordinates, partitions, add):
    res_partitions = [Partition(Coordinate(0, 0), 0) for i in range(0, len(partitions))]
    for coordinate in coordinates:
        res_partitions[coordinate.partition-add].centroide.d1 += coordinate.d1
        res_partitions[coordinate.partition-add].centroide.d2 += coordinate.d2
...
for i in range(0, it):
    for coordinate in coordinates:
        associa_objeto(coordinate, partitions, add)
    partitions = recalcula_centroides(coordinates, partitions, add)
